        ...
        <div className={`relative px-6 py-2.5 rounded-[2rem] transition-all duration-500 bg-white/20 dark:bg-white/5 backdrop-blur-3xl border border-white/40 dark:border-white/10 shadow-2xl ${
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              {user ? (
                <div className="flex items-center gap-4">
                  {/* Dashboard and Sign Out removed as per request */}
                  <Link
                    to="/settings"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="lg:hidden bg-white dark:bg-obsidian-800 border-t border-gray-100 dark:border-white/5 overflow-hidden"
          >
            <div className="container mx-auto px-6 py-8 flex flex-col gap-6">
              {navLinks.map((link) => (
                <Link
                  ...
                  className="text-2xl font-display font-extrabold text-gray-900 dark:text-white"
                >
                  {link.name}
                </Link>
              ))}
              {user ? (
                <div className="mt-4 pt-6 border-t border-gray-100 dark:border-white/5">
                  <Link
                    to="/settings"
                    onClick={() => setIsOpen(false)}
                    className="flex items-center gap-3 p-4 rounded-2xl bg-mint/5 text-mint font-bold"
                  >
                    <div className="w-10 h-10 rounded-xl bg-mint flex items-center justify-center text-white">
                      {user?.name?.[0]?.toUpperCase()}
                    </div>
                ...
                <div className="flex flex-col gap-4 mt-4 pt-6 border-t border-gray-100 dark:border-white/5">
                  <Link to="/login" onClick={() => setIsOpen(false)} className="text-xl font-bold text-gray-600 dark:text-gray-400">
                    Sign In
                  </Link>
